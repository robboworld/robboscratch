...
public class ProjectIO {
        ...
        public function encodeProjectAsZipFile(proj:ScratchStage):ByteArray {
           trace("SAVE: Zip!");
                ...
                trace("SAVE: savePenLayer()");
                proj.savePenLayer();

                trace("SAVE: updateInfo()");
                proj.updateInfo();

                trace("SAVE: recordImagesAndSounds()");
                recordImagesAndSounds(proj.allObjects(), false, proj);

                var zip:ZipIO = new ZipIO();

                trace("SAVE: zip.startWrite()");
                zip.startWrite();
                addJSONData('project.json', proj, zip);

                trace("SAVE: addImagesAndSounds(zip)");
                addImagesAndSounds(zip);

                trace("SAVE: proj.clearPenLayer()");
                proj.clearPenLayer();

                trace("SAVE: zip.endWrite()");
                return zip.endWrite();
        }
