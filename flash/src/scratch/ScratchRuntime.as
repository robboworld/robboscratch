...
//----------
   ...
   private function initLoadChecker():void{
      var request:URLRequest = new URLRequest("http://127.0.0.1:9876/dialog/load/check?timer=" + getTimer());
      request.method = URLRequestMethod.GET;
      var loader:URLLoader = new URLLoader();

      loader.addEventListener(Event.COMPLETE, loadCheckOk);

      loader.load(request);
   }
   private function loadResetOk(event:Event):void{
      initLoadChecker();
   }



   private function loadCheckOk(event:Event):void
   {
      try{
         ...
         if(loader2.data.length == 0){
            initLoadChecker();
            trace("Load: File is not ready yet.");
         }
         else{
