...
public class BlockMenus implements DragClient {
   ...
   public static function BlockMenuHandler(evt:MouseEvent, block:Block, blockArg:BlockArg = null, menuName:String = null):void {
      ...
      if (menuName == 'robotDirection')  menuHandler.robotDirectionMenu(evt);
      if (menuName == 'motorLeftSpeed')  menuHandler.motorLeftSpeedMenu(evt);
      if (menuName == 'motorRightSpeed') menuHandler.motorRightSpeedMenu(evt);
...
//       handler.scrollAlignMenu(evt);
         ...
         handler.analogMenuRobot(evt);
         handler.brightMenuRobot(evt);
         handler.analogMenuLab(evt);
      }
      return [
   ...
   private function motorDirectionMenu(evt:MouseEvent):void {
      var ops:Array = ['forward', 'backward'];
      var m:Menu = new Menu(setBlockArg, 'motorDirection');
      for each (var s:String in ops) m.addItem(s);
      showMenu(m);
   }
   private function robotDirectionMenu(evt:MouseEvent):void {
      var ops:Array = ['forward', 'backward', 'turn left', 'turn right'];
      var m:Menu = new Menu(setBlockArg, 'motorDirection');
      for each (var s:String in ops) m.addItem(s);
      showMenu(m);
   }
   ...
   private function motorLeftSpeedMenu(evt:MouseEvent):void {
      var speed:int;
      var m:Menu = new Menu(setBlockArg, 'motorLeftSpeed');
      for (speed = -100; speed < 100; speed++) m.addItem(speed);
      showMenu(m);
   }

   private function motorRightSpeedMenu(evt:MouseEvent):void {
      var speed:int;
      var m:Menu = new Menu(setBlockArg, 'motorRightSpeed');
      for (speed = -100; speed < 100; speed++) m.addItem(speed);
      showMenu(m);
   }


   private function ledMenu(evt:MouseEvent):void {
      var ops:Array = [0, 1, 2, 3, 4, 5, 6, 7];
      var m:Menu = new Menu(setBlockArg, 'selectLed');
      for each (var s:int in ops) m.addItem(s);
      showMenu(m);
   }
   private function ledColorMenu(evt:MouseEvent):void {
      var ops:Array = ['red', 'yellow', 'green'];
      var m:Menu = new Menu(setBlockArg, 'selectColorLed');
      for each (var s:String in ops) m.addItem(s);
      showMenu(m);
   }
   private function robotLedMenu(evt:MouseEvent):void {
      var m:Menu = new Menu(setBlockArg, 'robotSelectLed');
      m.addItem("position 1");
      m.addItem("position 2");
      m.addItem("position 3");
      m.addItem("position 4");
      m.addItem("position 5");
      showMenu(m);
   }
   ...
   private function notePickerLab(evt:MouseEvent):void {
      var piano:Piano = new Piano(block.base.color, app.viewedObj().instrument, setBlockArg);
      if (!isNaN(blockArg.argValue)) {
         piano.selectNote(int(blockArg.argValue));
      }
   ...
   private function analogMenuRobot(evt:MouseEvent):void {
...
*/
   ...
   private function analogMenuLab(evt:MouseEvent):void {
      var m:Menu = new Menu(setBlockArg, 'analogLab');
      m.addItem('light sensor');
      m.addItem('sound sensor');
      m.addItem('slider');
      showMenu(m);
   }
   ...
   private function getDigitalMenuLabRaw(evt:MouseEvent):void {
      var m:Menu = new Menu(setBlockArg, 'difitalLabRaw');

      for(var f:int = 8; f < 14; f++){
         m.addItem('D' + f);
      }
   ...
   private function setDigitalMenuLabRaw(evt:MouseEvent):void {
      var m:Menu = new Menu(setBlockArg, 'difitalLabRaw');
   ...
   private function setDigitalMenuLabPwmRaw(evt:MouseEvent):void {
      var m:Menu = new Menu(setBlockArg, 'difitalLabRaw');
   ...
   private function labButtons(evt:MouseEvent):void {
      var m:Menu = new Menu(setBlockArg, 'labButtons');

      for(var f:int = 1; f < 6; f++){
         m.addItem('' + f);
      }
   ...
   private function on_off(evt:MouseEvent):void {
      var m:Menu = new Menu(setBlockArg, 'On_Off');
   ...
   private function menu0_255(evt:MouseEvent):void {
      var m:Menu = new Menu(setBlockArg, 'menu0_255');
