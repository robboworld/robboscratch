...
spriteFeaturesFilter.visible = false; // disable features filter for now
   ...
   private function viewLibrary():void {
      ...
      trace("AZ: assetType=" + assetType);

      //if (!libraryCache) app.server.getMediaLibrary(gotLibraryData, assetType);
      //else collectEntries();

      app.server.getMediaLibrary(gotLibraryData, assetType);
   }
