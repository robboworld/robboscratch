...
//    primTable['yScroll']          = function(b:*):* { return app.stagePane.yScroll };
      ...
      primTable["ledOn:"]              = ledOn;
      primTable["ledOff:"]             = ledOff;

      primTable["ledColorOn:"]         = ledColorOn;
      primTable["ledColorOff:"]        = ledColorOff;

      primTable["robotLedOn:"]         = robotLedOn;
      primTable["robotLedOff:"]        = robotLedOff;

   }


   private function ledOn(b:Block):void {
      var led:int = interp.arg(b, 0);

      if (interp.activeThread.firstTime) {
         interp.startTimer(0.05);
         interp.redraw();

         app.robotCommunicator.ledOn(led);
      }
      else if (interp.checkTimer()){
      }
   }
   private function ledOff(b:Block):void {
      var led:int = interp.arg(b, 0);
      ...
      if (interp.activeThread.firstTime) {
         ...
         app.robotCommunicator.ledOff(led);
      }
      else if (interp.checkTimer()){
      }
   }
   private function ledColorOn(b:Block):void {
      var led:String = interp.arg(b, 0);
      ...
      if (interp.activeThread.firstTime) {
         ...
         if(led == 'red'){
            app.robotCommunicator.ledColorOn('r');
         }
         if(led == 'yellow'){
            app.robotCommunicator.ledColorOn('y');
         }
         if(led == 'green'){
            app.robotCommunicator.ledColorOn('g');
         }
      }
      else if (interp.checkTimer()){
      }
   }
   private function ledColorOff(b:Block):void {
      var led:String = interp.arg(b, 0);
      ...
      if (interp.activeThread.firstTime) {
         ...
         if(led == 'red'){
            app.robotCommunicator.ledColorOff('r');
         }
         if(led == 'yellow'){
            app.robotCommunicator.ledColorOff('y');
         }
         if(led == 'green'){
            app.robotCommunicator.ledColorOff('g');
         }
      }
      else if (interp.checkTimer()){
      }
   ...
   private function robotLedOn(b:Block):void {
      if (interp.activeThread.firstTime) {
         interp.startTimer(0.05);
         interp.redraw();
         ...
         var led:String = interp.arg(b, 0);
         if(led == "position 1"){
            app.robotCommunicator.robLedOn(1);
         }
         if(led == "position 2"){
            app.robotCommunicator.robLedOn(2);
         }
         if(led == "position 3"){
            app.robotCommunicator.robLedOn(3);
         }
         if(led == "position 4"){
            app.robotCommunicator.robLedOn(4);
         }
         if(led == "position 5"){
            app.robotCommunicator.robLedOn(5);
         }
      }
      else if (interp.checkTimer()){
      }
   }
   private function robotLedOff(b:Block):void {
      if (interp.activeThread.firstTime) {
         interp.startTimer(0.05);
         interp.redraw();
         ...
         var led:String = interp.arg(b, 0);
         if(led == "position 1"){
            app.robotCommunicator.robLedOff(1);
         }
         if(led == "position 2"){
            app.robotCommunicator.robLedOff(2);
         }
         if(led == "position 3"){
            app.robotCommunicator.robLedOff(3);
         }
         if(led == "position 4"){
            app.robotCommunicator.robLedOff(4);
         }
         if(led == "position 5"){
            app.robotCommunicator.robLedOff(5);
         }
      }
      else if (interp.checkTimer()){
      }
